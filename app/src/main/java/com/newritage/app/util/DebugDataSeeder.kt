...
object DebugDataSeeder {
    ...
    /**
     * 매듭 보관함에 10종 매듭이 전부 보이도록, 달마다 서로 다른 매듭으로 추천되게 하는 감정 문장.
    ...
    private val KNOT_SAMPLE_EMOTIONS = listOf(
        ...
        "가족과 함께한 시간이 행복하고 감사했다",                    // butterfly     -> 나비매듭
        "생활의 균형과 조화를 지키며 건강한 하루를 보냈다",          // samjeongja    -> 삼정자매듭
        "친구와 오해를 풀고 화해하며 서로 소통하고 공감했다",        // glasses       -> 안경매듭
        "동료의 응원과 약속이 고마웠고 서로의 관계가 깊어졌다",      // ring          -> 가락지매듭
        "설레고 기대되는 새로운 경험에 활기와 에너지가 넘쳤다",      // chick         -> 병아리매듭
        "중심을 잡고 침착하게 자신감 있고 든든한 하루였다"           // gaji_bangseok -> 가지방석매듭
    )
